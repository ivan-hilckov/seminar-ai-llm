...
/**
 * Реестр всех слайдов Секции 1 (02–49 · 48 слайдов · 90 мин).
...
export const slides = [
  ...
  ready('29', 'C', 'Откуда взялось пространство смыслов', '2.2 Обучение и заморозка', Slide29),
  ready('30', 'C', 'После обучения пространство фиксируется', '2.2 Обучение и заморозка', Slide30),

  // 2.3 Контекст и память
  ready('31', 'C', 'Что такое контекст', '2.3 Контекст и память', Slide31),
  ready('32', 'B', 'Модель не помнит — перечитывает', '2.3 Контекст и память', Slide32),
  ready('33', 'C', 'Контекстное окно', '2.3 Контекст и память', Slide33),

  // 2.4 Как рождается ответ
  ready('34', 'C', 'Как выбирается слово', '2.4 Как рождается ответ', Slide34),
  ready('35', 'D', 'Тот же вопрос — разный ответ', '2.4 Как рождается ответ', Slide35),

  // 3.0 Заставка
  ready('36', 'A', 'Особенности в работе', '3.0 Заставка', Slide36),

  // 3.1 Данные и приватность
  ready('37', 'D', 'Облачно vs локально', '3.1 Данные и приватность', Slide37),
  ready('38', 'C', 'Как отключить обучение на ваших данных', '3.1 Данные и приватность', Slide38),

  // 3.2 Память и контекст в работе
  ready('39', 'C', 'Память (Memory) — отдельная функция', '3.2 Память и контекст в работе', Slide39),
  ready('40', 'B', 'Когда Память полезна, когда мешает', '3.2 Память и контекст в работе', Slide40),
  ready('41', 'D', 'Новый чат — новый лист', '3.2 Память и контекст в работе', Slide41),

  // 3.3 Доступ к интернету
  ready('42', 'C', 'Поиск — отдельная функция', '3.3 Доступ к интернету', Slide42),

  // 3.4 Точность и галлюцинации
  ready('43', 'B', 'Не знает фактов — знает статистику текстов', '3.4 Точность и галлюцинации', Slide43),
  ready('44', 'D', 'Как выглядит галлюцинация', '3.4 Точность и галлюцинации', Slide44),
  ready('45', 'D', 'У думающих моделей выше галлюцинации', '3.4 Точность и галлюцинации', Slide45),
  ready('46', 'D', 'Где проверять обязательно', '3.4 Точность и галлюцинации', Slide46),

  // 3.5 Границы применения
  ready('47', 'B', 'AI берёт на себя задачи, не роль', '3.5 Границы применения', Slide47),
  ready('48', 'B', 'Она понимает буквально — не интуитивно', '3.5 Границы применения', Slide48),

  // Итог первой половины (три колонки по главам + таймлайн истории LLM)
  ready('49', 'C', 'Что мы прошли', 'Итог', Slide49),
];
 ...
 * Итоговый слайд «Что мы прошли» (46) входит в Часть III.
 */
