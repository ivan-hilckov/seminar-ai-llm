...
export const slides = [
  ...
  ready('21', 'D', 'У думающих моделей выше галлюцинации', '1.5 Думающие модели', Slide21),

  // 1.6 Главный вывод
  ready('22', 'C', 'От слова к рассуждению', '1.6 Главный вывод', Slide22),
  ready('23', 'E', 'Это не интеллект. Это статистика.', '1.6 Главный вывод', Slide23, { keystone: true }),

  // 2.0 Заставка
  ready('24', 'A', 'Как это работает внутри', '2.0 Заставка', Slide24),

  // 2.1 Векторы смыслов
  ready('25', 'B', 'Откуда модель знает, что «врач» и «доктор» близки?', '2.1 Векторы смыслов', Slide25),
  ready('26', 'C', 'Пространство смыслов', '2.1 Векторы смыслов', Slide26),
  ready('27', 'B', 'Близость = смысл', '2.1 Векторы смыслов', Slide27),

  // 2.2 Контекст и память
  ready('28', 'C', 'Что такое контекст', '2.2 Контекст и память', Slide28),
  ready('29', 'D', 'Новый чат — новый лист', '2.2 Контекст и память', Slide29),
  ready('30', 'C', 'Память (Memory) — отдельная функция', '2.2 Контекст и память', Slide30),

  // 2.3 Обучение и заморозка
  ready('31', 'B', 'Модель обучают один раз', '2.3 Обучение и заморозка', Slide31),

  // 3.0 Заставка
  ready('32', 'A', 'Особенности в работе', '3.0 Заставка', Slide32),

  // 3.1 Данные и приватность
  ready('33', 'D', 'Облачно vs локально', '3.1 Данные и приватность', Slide33),
  ready('34', 'C', 'Как отключить обучение на ваших данных', '3.1 Данные и приватность', Slide34),

  // 3.2 Память и контекст в работе
  ready('35', 'B', 'Когда Память полезна, когда мешает', '3.2 Память и контекст в работе', Slide35),

  // 3.3 Доступ к интернету
  ready('36', 'C', 'Поиск — отдельная функция', '3.3 Доступ к интернету', Slide36),

  // 3.4 Точность и галлюцинации
  ready('37', 'B', 'Не знает фактов — знает статистику текстов', '3.4 Точность и галлюцинации', Slide37),
  ready('38', 'D', 'Как выглядит галлюцинация', '3.4 Точность и галлюцинации', Slide38),
